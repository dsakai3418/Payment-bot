...
if user_input := st.chat_input("ここに入力してください..."):
    ...
    パターンB：確認したいことがある / 質問がある / わからない / 担当と話したい / 当月払えない場合
      ...
      ★重要判定: ユーザーが「やだ」「いや」「違う」「ダメ」「変更したい」等の否定的な言葉、または「新しいメアド」を入力した場合は、
      ステップ3（最終確認）には進まず、「恐れ入ります。ご希望のメールアドレスをご入力いただけますでしょうか？」と聞き返してください。
      新しいメアドが入力されたら、それを使ってステップ3（最終確認）に進んでください。

      肯定（はい、OK、お願いします）の場合のみ、以下の最終確認を出してください。
      ...
      ★ステップ4（追記・修正対応）:
      もし「ステップ3（最終確認）」を出力した後に、ユーザーがさらに何か発言した場合、以下のルールで対応してください。
      
      1. 「メアドを変えたい」「アドレスが違う」等の場合：
         「承知いたしました。新しいメールアドレスをご入力ください。」と聞き返し、入力されたら[ヒアリングした内容]は保持したまま、メアドだけ更新して再度ステップ3を出力してください。
         
      2. 質問内容を追加したい場合：
         以前の[ヒアリングした内容]に新しい発言を追記して、再度ステップ3を出力してください。
